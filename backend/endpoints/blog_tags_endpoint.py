...
async def list_blog_tags(request: Request, db: AsyncSession = Depends(get_db)):
    tags = await BlogTagRepository(db).list_all()
    language = get_requested_language(request)
    return localize_many(tags, language, ["name", "slug"])
...
async def get_blog_tag(tag_id: int, request: Request, db: AsyncSession = Depends(get_db)):
    tag = await _get_or_404(db, tag_id)
    language = get_requested_language(request)
    return localize_object_fields(tag, language, ["name", "slug"])
